...
import { setError, setProjects } from "@/store/slice/projectSlice";
...
export default function App() {
  ...
  return (
    ...
    >
      ...
      {/* Main content */}
      <main className="pt-[20px] px-4 md:px-8 lg:px-10 max-w-[1440px] mx-auto pb-16">
        {/* ─── Hero ─── */}
        <SectionWrapper className="mt-8 mb-6 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          ...
          <button
            className="self-start flex items-center gap-2 px-4 py-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] text-sm font-medium hover:bg-gray-50 dark:hover:bg-white/5 transition-colors shadow-sm"
            style={{ color: "var(--color-text)" }}
          >
        ...
        <SectionWrapper
          delay={100}
          className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr] gap-5 mb-6"
        >
          <ProjectOverview />
          <RecentActivity />
          <TopProjects />
        ...
        <SectionWrapper
          delay={150}
          className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-[1.6fr_1.2fr_.9fr] gap-5 mb-6"
        >
          <TeamProductivity />
          <TasksByPriority />
          <AchievementCard />
