...
const TaskRow = memo(function TaskRow({ task, delay = 0, onToggle, dropdown, menuId }) {
  ...
  return (
    ...
    >
      {/* Priority strip */}
      <div className="task-priority-strip" style={{ background: pc.strip }} />
      ...
      <div className="flex-1 min-w-0">
        <p
          className={`text-sm font-semibold leading-snug ${task.completed ? 'task-title-strike opacity-60' : ''}`}
          style={{ color: 'var(--color-text)' }}
        >
          {task.name}
        </p>
        <p className="text-xs mt-0.5 truncate hidden sm:block" style={{ color: 'var(--color-muted)' }}>
          {task.description}
        </p>
      </div>
      ...
      {/* Meta: comments, attachments, estimate */}
      <div className="hidden xl:flex items-center gap-4 shrink-0">
        <div className="task-meta-icon flex items-center gap-1 text-xs" style={{ color: 'var(--color-muted)' }}>
